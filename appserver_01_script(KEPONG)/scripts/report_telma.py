...
def smpp_logs_writing(config):
    ...
    for instance_label, path in instances.items():
        try:
            mod_time = datetime.datetime.fromtimestamp(os.path.getmtime(path))
            time_diff = (now - mod_time).total_seconds() / 60  # in minutes

            # If file modified within last 5 minutes, consider it writing
            writing_status = "writing" if time_diff <= 5 else "not writing"
            expected_status = "writing" if "2875" not in instance_label else writing_status

            output = f"{instance_label:<30} | {writing_status:<12} | {expected_status}"
        except Exception:
            fallback_status = "not writing"
            expected_status = fallback_status if "2875" in instance_label else "writing"
            output = f"{instance_label:<30} | {fallback_status:<12} | {expected_status}"
...
def smpp_error_count(config):
    ...
    for instance_name, log_path in instances.items():
        try:
            cmd = f'grep -c "\\[ERROR\\] {today_str}" "{log_path}"'
        ...
        # Append 'ERROR' to the instance name with spacing
...
def ss7_trace_count(config):
    ...
    latency_threshold = trace_conf.get("latency_threshold", 2000)  # default is 2000 if not provided
    ...
    for i in range(1, file_count + 1):
        ...
        if not os.path.isfile(full_path):
            output_lines.append(f"SS7 Trace File {i:<2} Latency              | File Missing | Skipped")
            continue

        try:
            # Total Latency:-1
            cmd_total = f"grep -c 'Latency:-1' \"{full_path}\""
            result_total = subprocess.run(cmd_total, shell=True, capture_output=True, text=True)
            total = int(result_total.stdout.strip()) if result_total.returncode == 0 else 0

            # Latency:-1 with response_code != 0
            cmd_nonzero = f"grep 'Latency:-1' \"{full_path}\" | awk -F'response_code\":' '{{if ($2+0 != 0) print $0}}' | wc -l"
            result_nonzero = subprocess.run(cmd_nonzero, shell=True, capture_output=True, text=True)
            nonzero = int(result_nonzero.stdout.strip()) if result_nonzero.returncode == 0 else 0

            # Latency:-1 with response_code == 0
            zero_rc = total - nonzero

            # Status formatting
            if zero_rc >= latency_threshold:
                status = "! Exceeds Limit"
            else:
                status = f"Not Exceed: {latency_threshold}"

            line = f"SS7 Trace File {i:<2} Latency              | {zero_rc:<8} | {status}"
            output_lines.append(line)

        except Exception as e:
            output_lines.append(f"SS7 Trace File {i:<2} Latency              | Error        | {str(e)}")
