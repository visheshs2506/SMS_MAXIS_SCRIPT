...
def check_jar_status(config):
    ...
    return [
        f"{'JAR Status for SIP':<30} | {running_count}/{expected_count:<3}       | Expected: {expected_count}/{expected_count}",
...
def check_smpp_cdr(config):
    try:
        ...
        dir_path = smpp_conf.get("watch_dir", "/data/armour/smpp/SIP_CDR/Folder1")
        ...
        csv_files = [line for line in lines if "Armour_SIP" in line and line.strip().endswith(".csv")]
        ...
        time_status = f"SIP CDR last {expected} file created   | {recent_count}/{expected:<3}       | Expected: {expected}/{expected}"
        write_status = f"SIP CDR last {expected} file writing   | {writing_count}/{expected:<3}       | Expected: {expected}/{expected}"

        return [time_status, write_status]

    except Exception as e:
        return [f"SIP CDR Monitor                | Error         | {str(e)}"]
...
def ss7_logs_writing(config):
    ...
    for i in range(1, trace_file_count + 1):
        filename = f"{filename_prefix}{i}-Trace-{now.strftime('%Y-%m-%d')}.log"
...
def smpp_error_count(config):
    ...
    for instance_name, inst in instances.items():
        ...
        label = f"SIP {instance_name}  ERROR"
...
def ss7_trace_count(config):
    ...
    for i in range(1, file_count + 1):
        filename = f"{prefix}{i}-Trace-{today_str}.log"
...
def armour_mt_response_count(config):
    ...
    state_dir = trace_config["state_file_dir"]
    filename_prefix = trace_config["filename_prefix"]
    patterns = trace_config["patterns"]

    today = datetime.datetime.now().strftime("%Y-%m-%d")

    os.makedirs(state_dir, exist_ok=True)
    state_file = os.path.join(state_dir, "armour_mt_offset_state.json")
    ...
    for pattern_conf in patterns:
        ...
        for file in files:
            ...
            matches = len(regex.findall(new_data))
            state[offset_key] = new_offset
            total_delta += matches

        sum_key = f"SUM::{label}"

        previous_total = state.get(sum_key, 0)
        current_total = previous_total + total_delta
        ...
        output_lines.append(
            f"{label:<35} | Current: {current_total:<10} | Previous: {previous_total} ({status})"
        )

        state[sum_key] = current_total
